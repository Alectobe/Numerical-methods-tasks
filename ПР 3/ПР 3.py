...
def nextx(x0, x1, a):
    funcs = func(x0, a)
    res = x0 - ((x1 - x0) * funcs / (func(x1, a) - funcs)) * c
    return res
...
c = 0.7
max_iter = 10000
answer = Secant(x0, x1, e)
print('x через хорды:', answer)
print(f'При x = {answer}, y = {func(answer, a):.20f}')
print()
...
fxs = [1, 2, -5, 8, -7, -3]
# print(R(fxs))
print(f'{1 / R1(fxs)} <= x-положительный <= {R(fxs)}')
print(f'{-1 * R2(fxs)} <= x-отрицательный <= {-1 / R3(fxs)}')
